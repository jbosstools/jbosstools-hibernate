...
//            }
  //          adapter = outlinePage;
    ...
    public ConnectionInfo requestConnectionFromUser() {
        ...
        Shell shell = 
            HQLEditorPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getShell();
    ...
    /*public void setConnectionInfo( ConnectionInfo connInfo ) {
        ...
        if (input instanceof IHQLEditorInput) {
            IHQLEditorInput sqlEditorInput = (IHQLEditorInput) input;
            sqlEditorInput.setConnectionInfo( connInfo );
        }
        
        if (fConnectionInfo != null) {
            fDBProposalsService = new HQLDBProposalsService( connInfo );
            SourceViewerConfiguration config = getSourceViewerConfiguration();
            if (config != null && config instanceof HQLSourceViewerConfiguration) {
                HQLSourceViewerConfiguration sqlConfig = (HQLSourceViewerConfiguration) config;
        ...
        if (editorSite != null) {
            ...
            if (actionBars != null) {
                ...
                if (connInfo != null) {
                    ...
                    String connStatus = HQLEditorResources.getString( "HQLEditor.connection.status.noConnection" ); //$NON-NLS-1$
