...
public class CoreMappingTestHelper {
	
	public CoreMappingTestHelper(TestName testName) {
	...
	private void doAfter() throws Exception {
		IPath cfgFilePath = new Path(
				TestProject.SRC_FOLDER + File.separator + ConsoleConfigUtils.CFG_FILE_NAME);
		IFile file = testProject.getIProject().getFile(cfgFilePath);
